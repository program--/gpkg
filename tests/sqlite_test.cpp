...
SCENARIO("User is accessing a SQLite Database")
{
    ...
    GIVEN("a path to the database")
    {
        ...
        THEN("user wants metadata about the database")
        {
            REQUIRE(db.has_table("gpkg_contents"));
            REQUIRE(!db.has_table("somefaketable"));
        }
